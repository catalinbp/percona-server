...
using keyring_common::utils::get_random_iterations;
...
bool Keyring_encrypted_file_backend::encrypt_data(const std::string &plaintext,
  ...
  if (aes_encrypt_pbkdf2(
          ...
          password_.length(), salt.get(), k_salt_size, iterations, mode,
  ...
  */
  ...
  const uint8_t iters_be[4] = {static_cast<uint8_t>((iterations >> 24) & 0xFF),
                               static_cast<uint8_t>((iterations >> 16) & 0xFF),
                               static_cast<uint8_t>((iterations >> 8) & 0xFF),
                               static_cast<uint8_t>(iterations & 0xFF)};
